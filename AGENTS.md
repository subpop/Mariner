...
Mariner is a native macOS Gemini-protocol browser built with SwiftUI. The
codebase is a single app target plus a remote Swift package:

- **Mariner/** -- App target (SwiftUI views, `BrowserState` engine, stores)
  - `App/` -- Entry point, commands, Settings scene
  - `Browser/` -- `BrowserState`, browser chrome, gemtext rendering, dialogs
  - `Stores/` -- Bookmarks, history, per-host settings (JSON persistence)
  - `Identities/` -- PKCS#12 client-identity import (Keychain-backed)
  - `Settings/` -- Settings tabs
- [**GeminiKit**](https://github.com/subpop/GeminiKit) -- Gemini transport, TOFU pins,
  gemtext parser, test server. Mariner owns all UI, bindings, and
  persistence; GeminiKit stays transport-only.

Views use the shared `@Observable` stores and `BrowserState` directly.
Only `BrowserState` (via injectable closures) and `SettingsView` touch
`GeminiClient`; views never import transport details beyond `GeminiKit`
model types (`GeminiURI`, `GemtextBlock`).

## Build & Test

- Open `Mariner.xcodeproj` in Xcode 26+.
- Build: `Cmd+B` with the **Mariner** scheme selected.
- Run tests: `Cmd+U` or `xcodebuild -scheme Mariner test`.
- Requires macOS 26.0 (Tahoe) or later.
- GeminiKit has its own suite: `swift test` in a `GeminiKit` checkout.
...
- **Model/service**: GeminiKit (transport), `Stores/AppStores` (JSON
  persistence), `Identities/ClientIdentityStore` (Keychain),
  `App/AppSettings` (UserDefaults-backed preferences).
- **View model**: `Browser/BrowserState` (the window's view model and
  fetch orchestrator), `Browser/FindModel` (find-in-page),
  `Browser/PageState` (page display states).
- **View**: `BrowserView`, `GemtextView`, `Dialogs`, `SettingsView` —
  rendering and callbacks only, no engine logic.

- Transport stays in GeminiKit; UI, identity import, per-host bindings,
  and all persistence live in Mariner. Do not add UI or persistence
  to GeminiKit — extend its `fetch` overloads instead.
...
- Stores persist JSON under Application Support/Mariner (AppSettings is
  the exception: UserDefaults-backed); TOFU pins and identities live in
  the Keychain. In-memory variants (`persistenceURL: nil`,
  `AppSettings(requestTimeout:autoFollowSameHostRedirects:)`, no
  identity store) are for previews and tests.
...
Mariner should look and feel like a first-class macOS app, not a cross-platform
